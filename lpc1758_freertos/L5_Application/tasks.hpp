...
/**
 * Terminal task is our uart2 terminal that handles our commands into the board.
 ...
 * It can "learn" remote control codes by typing "learn" into the uart2 terminal.
...
class uart2_hw1 : public scheduler_task
{
    public:
       uart2_hw1(uint8_t priority);
       bool uart2_hw1_init(void);
       char uart2_outchar(char);
       char uart2_inchar();
       bool run(void *p);
       bool taskEntry(void);
...
class spi1_hw2 : public scheduler_task
{
    public:
        spi1_hw2(uint8_t priority);
        bool taskEntry(void);
        bool run(void *p);
        bool spi1_hw2_init(void);
        uint8_t spi1_byte(uint8_t);
    private:
        void selectFlash();
        void readFlash();
        void waitFlashReady();
...
class i2c_hw3 : public scheduler_task
{
    public:
        bool run(void *p);
        bool taskEntry(void);
        i2c_hw3(uint8_t priority);
};

class Led_Switch : public scheduler_task
{
    public:
        bool run(void *p);
        Led_Switch(uint8_t priority);
        bool taskEntry(void);
};
