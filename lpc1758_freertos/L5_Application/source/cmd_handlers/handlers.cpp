...
CMD_HANDLER_FUNC(taskListHandler)
{
    ...
    output.printf("%10s St Pr  Stack  CPU%%           Time\n", "Name", TIMER0_US_PER_TICK);
    for(unsigned portBASE_TYPE i = 0; i < uxArraySize; i++) {
        ...
        uint64_t timeUs = (uint64_t) e->ulRunTimeCounter * TIMER0_US_PER_TICK;
        uint32_t timeMs = timeUs / 1000;
        unsigned int time = timeMs > 0 ? timeMs : timeUs;
        const char *pUnits = timeMs > 0 ? "ms" : "us";

        output.printf("%10s  %c %2u  %5u  %4u  %10u %s\n",
                      e->pcTaskName, taskStatusTbl[e->eCurrentState], e->uxBasePriority,
                      e->usStackHighWaterMark, cpuPercent,
                      time, pUnits);
    }
...
CMD_HANDLER_FUNC(i2cIoHandler)
{
    ...
    if (read) {
        ...
        for (int addr = 2; addr <= 254; addr += 2) {
            if (I2C2::getInstance().isDevicePresent(addr)) {
...
CMD_HANDLER_FUNC(producerTask)
{
    scheduler_task *compute = NULL;
    scheduler_task *temp = NULL;
    temp = compute->getTaskPtrByName("compute");
    if(cmdParams == "suspend")
    {
        temp ->suspend();
        output.putline("Producer task Suspended");
    }
    else if (cmdParams == "resume")
    {
        vTaskResume(temp->getTaskHandle());
        output.putline("Producer task Resume");
    }
    else
    {
        return false;
